#include <arpa/inet.h>
#include <errno.h>
#include <pthread.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <sys/socket.h>
#include <unistd.h>

#define BUF_SIZE 64
#define PORT 8000
#define LISTEN_BACKLOG 32

#define handle_error(msg)                                                      \
  do {                                                                         \
    perror(msg);                                                               \
    exit(EXIT_FAILURE);                                                        \
  } while (0)

// Shared counters for: total # messages, and counter of clients (used for
// assigning client IDs)
int total_message_count = 0;
int client_id_counter = 1;

// Mutexs to protect above global state.
pthread_mutex_t count_mutex = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t client_id_mutex = PTHREAD_MUTEX_INITIALIZER;

struct client_info {
  int cfd;
  int client_id;
};

void *handle_client(void *arg) {
  struct client_info *client = (struct client_info *)arg;
  char buffer[BUF_SIZE];
  int bytes_read = 0;

  // TODO: print the message received from client
  while ((bytes_read = read(client->client_id, buffer, BUF_SIZE)) > 0 &&
         errno != EAGAIN) {
    printf("New client created! ID %d on socket FD %d\n", client->client_id,
           client->cfd);

    buffer[bytes_read] = '\0'; // Null terminate client message
    pthread_mutex_lock(&count_mutex);

    // TODO: increase total_message_count per message
    total_message_count++;
    printf("Msg #   %d; Client ID %d: %s/n", total_message_count,
           client->client_id, buffer);

    pthread_mutex_unlock(&count_mutex);
  }

  // Error handle
  if (bytes_read == -1) {
    perror("read");
    exit(EXIT_FAILURE);
  }

  // Free dynamic memeory and close client
  printf("Ending thread for client %d\n", client->client_id);
  free(client);
  close(client->cfd);
  return NULL;
}

int main() {
  struct sockaddr_in addr;
  int sfd;

  sfd = socket(AF_INET, SOCK_STREAM, 0);
  if (sfd == -1) {
    handle_error("socket");
  }

  memset(&addr, 0, sizeof(struct sockaddr_in));
  addr.sin_family = AF_INET;
  addr.sin_port = htons(PORT);
  addr.sin_addr.s_addr = htonl(INADDR_ANY);

  if (bind(sfd, (struct sockaddr *)&addr, sizeof(struct sockaddr_in)) == -1) {
    handle_error("bind");
  }

  if (listen(sfd, LISTEN_BACKLOG) == -1) {
    handle_error("listen");
  }

  for (;;) {
    // TODO: create a new thread when a new connection is encountered
    int cfd;
    cfd = accept(sfd, NULL, NULL); // accept anyone
    if (cfd == -1) {
      perror("accept");
      exit(EXIT_FAILURE);
    }

    pthread_mutex_lock(&client_id_mutex);
    int client_id_count = client_id_counter++;
    pthread_mutex_unlock(&client_id_mutex);

    struct client_info *info = malloc(sizeof(struct client_info));
    info->cfd = cfd;
    info->client_id = client_id_count;

    // TODO: call handle_client() when launching a new thread, and provide
    // client_info
    pthread_t thread;
    if (pthread_create(&thread, NULL, handle_client, info) != 0) {
      perror("pthread_create");
      exit(EXIT_FAILURE);
    }
    pthread_detach(thread);
  }

  if (close(sfd) == -1) {
    handle_error("close");
  }

  return 0;
}
