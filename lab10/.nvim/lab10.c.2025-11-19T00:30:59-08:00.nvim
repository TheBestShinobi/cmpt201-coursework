...
1. How is the client sending data to the server? What protocol?
  Sending data by connecting to the server using the connect
  function which is TCP.
2. What data is the client sending to the server?
  the data being sent to the server are words in a global array of
  strings called messages.

Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
  the argument is a struct include various things such as a bool
  variable to check if the server is still running, a struct to a
  list which will be used to store the client strings and paste them,
  and a mutex to be thread safe when accessing the list items.
2. How are received messages stored?
  they are stored in a linked list with struct list_handle
3. What does `main()` do with the received messages?
  it runs the function 'collect_all' on them which prints out each
  node data that was collected and main makes sure that all data was
  collected and prints if it was or wasn't
4. How are threads used in this sample code?
  they are used to create an acceptor thread which handles accepting
  new clients by creating even more threads for each client to handle
  their request/data being inputted into the linked list


5.
How are sockets made non-blocking:
  through the set_non_blocking function created which internally uses
  the 'fcntl()' function to specify a FD to be non-blocking

What sockets are make non-blocking:
  all the client sockets are made to be non-blocking in the
  'run_client' thread function. and the server socket is also
  set to non-blocking in the 'run_acceptor' thread function

Why are these sockets made non-blocking? What purpose does it serve:
  Its used on the server socket to allow it to continueously accept
  clients rather than have to stop accepting clients if there is
  already one accepted and wait for it to fully go through its whole
  process. The clients sockets are made non-blocking too so when
  reading data from it, the program wont wait forever to read if
...
int main() {
  ...
  for (;;) {
    // Calculate now many messages there are
