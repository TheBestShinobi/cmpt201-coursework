...
int main() {
  ...
  if (file == NULL) {
    perror("Could not open the public_key.pem file");
    exit(EXIT_FAILURE);
  }
...
#define MAX_FILE_SIZE 512
  ...
  // Initialize verify EVP with the public key
  int val = EVP_DigestVerifyInit(mdctx, NULL, EVP_sha256(), NULL, pubkey);
  if (val != 1) {
    perror("EVP_DigestVerifyInit");
    exit(EXIT_FAILURE);
  }

  // Update the digest verification
  if (EVP_DigestVerifyUpdate(mdctx, message, msg_len) != 1) {
    perror("EVP_DigestVerifyUpdate");
    exit(EXIT_FAILURE);
  }

  // Finialize the verification to check if it was a success
  int result = EVP_DigestVerifyFinal(mdctx, signature, sign_len);

  if (result == 1) {
    return 1;
  }
  // Handle if the verification failed
  else if (result == 0) {
    return 0;
  }
  // Handle if the message does not match the signal
  else {
    return -1;
  }
